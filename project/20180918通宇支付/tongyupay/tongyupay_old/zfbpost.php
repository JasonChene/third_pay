...
$bizContent = array(
  ...
  "pay_platform" => '', //支付平台
  "pay_type" => '', //交易类型
  "payment_fee" => number_format($_REQUEST['MOAmount'] * 100, 0, '.', ''), //总金额
  "cur_type" => 'CNY', //币种
  "body" => 'pay', //商品名称
  "notify_url" => $merchant_url, //通知地址
  "bill_create_ip" => getClientIp(), //终端IP
...
ksort($bizContent);
$data['biz_content'] = stripslashes(json_encode($bizContent));
$signtext = 'biz_content=' . $data['biz_content'] . '&key=' . $pay_mkey;
...
if ($row['ret_code'] != '0') {
  ...
  $qrcodeUrl = $row['biz_content']['mweb_url'];
  if (_is_mobile()) {
    $jumpurl = $qrcodeUrl;
  } else {
