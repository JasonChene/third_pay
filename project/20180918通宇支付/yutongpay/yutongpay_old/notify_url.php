...
foreach ($res['biz_content'] as $key => $value) {
	$data[$key] = $value;
	//write_log($key . "=" . $value);
}

#设定固定参数
$order_no = $data['out_order_no']; //订单号
$mymoney = number_format($data['payment_fee'] / 100, 2, '.', ''); //订单金额
$success_msg = $res['ret_code'];//成功讯息
...
#验签方式
ksort($bizContent);
$bizContent = stripslashes(json_encode($res['biz_content']));
$signtext = 'biz_content=' . $bizContent . '&key=' . $pay_mkey;
