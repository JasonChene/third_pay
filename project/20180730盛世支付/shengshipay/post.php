...
#第三方参数设置
$data = array(
  "mch_appid" => $pay_mid, //商户号
  "mch_order_number" => $order_no, //商户订单号，在商户系统唯一
  "pay_type" => '', //支付方式
  "pay_money" => number_format($_REQUEST['MOAmount'] * 100, 0, '.', ''), //支付金额，单位分，无小数点
  "order_desc" => 'desc', //商品描述
  "notify_url" => $merchant_url, //接收支付结果异步通知的url 
  "return_url" => $return_url, //前台通知地址
  "bank_code" => '', //网关支付必填
  "sign" => '', //签名
);

#变更参数设置
$form_url = 'http://113.10.156.239:8081/pay';//扫码提交地址
if (strstr($pay_type, "银联快捷")) {
  $scan = 'ylkj';
  $data['pay_type'] = 'kuai_pay';
} else {
  $scan = 'wy';
  $data['pay_type'] = 'net_pay';
...
#跳转
  if (empty($row['payUrl'])) {
    echo '错误代码:' . $row['retCode'] . "\n";
    echo '错误讯息:' . $row['retMsg'] . "\n";
    exit;
  } else {
