...
<?php
	...
	$reserve = "";										//商户保留信息
	$pay_mode = "07";									//支付模式
	$bank_code = "QUICKPAY";							//银行编号
	$card_type = "0";									//允许支付的银行卡类型
	$merchant_user_id = "1";							//商户平台会员ID（建议用数据库中对应的会员数据记录的自增列（mysql）或序列(oracle)的值）
    ...
    $src = "version=" . $version . "&merchant_no=" . $merchant_no . "&order_no="
            ...
            . $card_type . "&merchant_user_id=" . $merchant_user_id;
    $src .= "&key=" . $key;
    $sign = md5($src);
...
<form action="<?=$url?>" method="POST" id="form">
	<table cellpadding="3">
		...
		<tr><td>商户平台会员ID：</td><td><input type="text" name="merchant_user_id" value="<?=$merchant_user_id?>" /></tr>
