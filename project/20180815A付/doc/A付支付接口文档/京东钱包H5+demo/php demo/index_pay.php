...
$ip=$_POST['ip']; 
	...
	$zd="notifyUrl=".$notifyUrl."&orderIp=".$orderIp."&orderPrice=".$orderPrice."&orderTime=".$orderTime."&outTradeNo=".$outTradeNo."&payKey=".$payKey."&productName=".$productName."&productType=".$productType."&remark=".$remark."&returnUrl=".$returnUrl."&paySecret=".$paySecret;  //拼接字符串，里面的拼接字段必须全部用上。
	
	$sign=strtoupper(md5($zd));    //MD5值必须大写
?>


<form method="post" action="https://gateway.aabill.net/cnpPay/initPay" id="myForm">
	...
	<input type="hidden" placeholder="支付产品名称" name="productName" value="<?=$productName?>">
