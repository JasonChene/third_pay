...
//支付接口地址
String api_url = 	StringUtils.formatString(EkaPayConfig.getInstance().getValue("card_url")); 	
/*
* 订单参数
*/
...
//将卡类型转换为中文说明
String chnType = EkaPayTypeConvert.cardTypeToChn(type); 

if(!Integer.valueOf(totalvalue).equals(sValue)){
	strResult = "提交的多张卡面值和总面值不相等，请充值提交";
}else{
	String sign = EkaPayEncrypt.EkaPayCardMultiMd5Sign(type,parter,cardno,cardpwd,value,totalvalue,restrict,    //签名
	...
	*/
	...
	//将支付上行结果转换为对应中文说明
	strResult = EkaPayTypeConvert.opstateValueToChn(result); 
}
%>
	<div>支付卡类接口演示(多卡)</div>
			<div>卡类型:<%=chnType%></div>
