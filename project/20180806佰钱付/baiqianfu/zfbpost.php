...
// 佰钱付參數設定
$form_url = "https://bq.baiqianpay.com/webezf/web/?app_act=openapi/bq_pay/pay"; // 提交地址
...
$X6_MD5info = "";
$X7_PaymentType = "ALIPAYSM"; // 不传默认网关支付
$X8_MerRemark = ""; // 备注
$isApp = "app";

if (_is_mobile()) {
  $X7_PaymentType = 'ALIPAYH5'; //手机微信
  $isApp = "";
}
...
$geturl = 'https://bq.baiqianpay.com/webezf/web/?app_act=openapi/bq_pay/pay&' . $data;
...
if (_is_mobile()) {
  ?>
    <html>
      ...
      <body>
        <form name="dinpayForm" method="post" id="frm1" action="<?php echo $form_url ?>" target="_self">
          <p>正在为您跳转中，请稍候......</p>
          <?php foreach ($parms2 as $arr_key => $arr_value) { ?>
    ...
    header('Location:' . '../qrcode/qrcode.php?type=zfb&code=' . $array["imgUrl"]);
  }
  ?>
