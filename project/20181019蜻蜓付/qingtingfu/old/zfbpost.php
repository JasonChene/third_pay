...
$form_url = 'http://47.244.4.133:3020/api/pay/create_order';//接入提交地址
...
if ($row['retCode'] != "SUCCESS") {
  ...
  if (_is_mobile()) {
    $jumpurl = $row['payParams']['codeUrl'];
  } else {
    $jumpurl = '../qrcode/qrcode.php?type=' . $scan . '&code=' . QRcodeUrl($row['payParams']['codeUrl']);
  }
}
?>
<html>
