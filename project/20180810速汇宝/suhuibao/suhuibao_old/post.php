...
if ($scan == "wy") {
  ...
  $form_url = 'https://pay.xhbill.com/gateway?input_charset=UTF-8';  //提交地址
  $interface_version = "V3.0";//接口版本
} elseif ($scan == "yl") {
  $service_type = 'union_scan';
...
$sign_type = "RSA-S";//签名法
$order_time = date("Y") . "-" . date("m") . "-" . date("d") . " " . date("H") . ":" . date("i") . ":" . date("s");//时间
...
if ($scan == "wy") {
  ...
  $parms['bank_code'] = $_REQUEST['bank_code'];
}
ksort($parms);
$signtext = '';
