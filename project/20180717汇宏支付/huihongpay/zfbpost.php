...
$data = array(
  ...
  "istype" => '', //支付类型:1（支付宝）、2（微信）
  "price" => number_format($_REQUEST['MOAmount'], 0, '.', ''), //商品价格(元)
  "mchno" => $pay_mid, //商户号
  "notify_url" => $merchant_url, //回调地址(通知url必须为直接可访问的url，不能携带参数)
  "sign" => '', //签名
);

#变更参数设置
$form_url = 'http://122.152.237.219/preCreate';//请求URL
$scan = 'zfb';
$data['istype'] = '1';
...
$signtext = mb_convert_encoding($signtext, "GBK", "UTF-8");
$sign = md5($signtext);
$data['sign'] = $sign;
$data_str['req'] = URLencode("<xml><mchno>" . $data['mchno'] . "</mchno><orderid>" . $data['orderid'] . "</orderid><istype>" . $data['istype'] . "</istype><price>" . $data['price'] . "</price><notify_url>" . $data['notify_url'] . "</notify_url><sign>" . $data['sign'] . "</sign></xml>");

#curl获取响应值
$res = curl_post($form_url, $data_str);
$resdata = simplexml_load_string(urldecode($res), 'SimpleXMLElement', LIBXML_NOCDATA);
$row = json_decode(json_encode($resdata), true);

#跳转
if ($row['success'] != 'true' || $row['code'] !== '0000') {
  echo '错误代码:' . $row['code'] . "\n";
