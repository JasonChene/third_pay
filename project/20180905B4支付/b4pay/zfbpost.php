...
#第三方参数设置
$data = array(
  "merchantId" => $pay_mid, //商户编码
  "timestamp" => number_format(microtime(true) * 1000, 0, '.', ''), //时间戳
  "tradeNo" => $order_no, //交易流水号
  "notifyUrl" => $merchant_url, //交易通知地址
  "totalAmount" => number_format($_REQUEST['MOAmount'], 2, '.', ''), //订单金额
...
foreach ($data as $arr_key => $arr_val) {
  if (!in_array($arr_key, $noarr) && (!empty($arr_val) || $arr_val === 0 || $arr_val === '0')) {
    $arr_val = urlencode($arr_val);
    $signtext .= $arr_key . '=' . $arr_val . '&';
  }
}
$signtext = substr($signtext, 0, -1);
$sign = hash_hmac("sha1", $signtext, $pay_mkey);
$data['signature'] = $sign;
...
if ($row['code'] != 1) {
  ...
  exit();
} else if ($row['data']['code'] != '10000') {
  echo '错误代码:' . $row['data']['code'] . "\n";
  echo '错误讯息:' . $row['data']['msg'] . "\n";
  exit;
} else {
  $qrcodeUrl = $row['data']['qr_code'];
