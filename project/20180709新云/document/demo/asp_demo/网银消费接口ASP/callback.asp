...
opstate = trim(request("opstate")) '新云支付返回商户状态
ovalue = trim(request("ovalue")) '新云支付返回实际面值
sign = trim(request("sign")) '签名
resulttime = trim(request("systime")) '新云支付处理完成时间
...
'opstate = -4,卡已经使用（卡在提交到新云支付联盟之前已经被使用）
if cint(opstate) = 0 then
'支付成功，可进行商户自身逻辑处理
	response.Write "opstate=0"'商户接收到新云支付的通知以后需要在页面上输出opstate=0，表示接收到通知，新云支付不会在继续进行通知了
	response.End()
else
'支付失败，可进行商户自身逻辑处理
	response.Write "opstate=0"'商户接收到新云支付的通知以后需要在页面上输出opstate=0，表示接收到通知，新云支付不会在继续进行通知了
