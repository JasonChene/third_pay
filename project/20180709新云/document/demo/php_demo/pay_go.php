 ...
 * 获取表单数据
 * */
$order_id = (string) date("YmdHis"); //您的订单Id号，你必须自己保证订单号的唯一性，新云支付不会限制该值的唯一性
...
//卡类支付
    ...
    switch ($result) {
        ...
        case '-3':
            ...
            break;
        case '-999':
            print('<script language="javascript">alert("对不起，新云支付接口维护中，请选择其他的充值方式！"); history.go(-1);</script>');
            break;
        default:
            print('未知的返回值, 请联系新云支付官方！');
            break;
    }
    ...
    switch ($result) {
        ...
        case '-2':
            ...
            break;
        case '-3':
            print('<script language="javascript">alert("卡密为重复提交，新云支付系统不进行消耗且不进入下行流程。"); history.go(-1);</script>');
            break;
        case '-4':
            print("卡密不符合新云支付定义的卡号密码面值规则，新云支付系统不进行消耗且不进入下行流程。");
            break;
        case '-999':
            print('<script language="javascript">alert("对不起，新云支付接口维护中，请选择其他的充值方式！"); history.go(-1);</script>');
            break;
