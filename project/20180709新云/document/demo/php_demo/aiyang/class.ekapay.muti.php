...
/*
  ...
  recive()
  接收新云支付消息，这里会进行身份的验证，如果您自己要做更多后续的处理，请在您自己的程序中进行
  调用示例：
...
class ekapay {
    const aiyang_card_url = 'http://openapi.7t1.cn/card/index.aspx';
    const aiyang_card_search_url = 'http://openapi.7t1.cn/card/index.aspx';
    ...
    public function send() {
        ...
        $error = 0;
        $msg = '您调用新云支付支付接口的参数有误，错误信息如下：';
        if (empty($this->parter)) {
            $error = 1;
            $msg .= '<li>parter不能为空: 商户id，由新云支付联盟分配</li>';
        }
        if (empty($this->type)) {
            $error = 1;
