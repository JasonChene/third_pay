...
if(isset($_GET['type']))
{
	...
	else if($_GET['type'] == "qq")
	{
	    $title = "QQ钱包扫一扫付款";
	    $openMobile = "打开手机QQ钱包";
	    $payComment = "QQ钱包在线充值说明：";
	}else if($_GET['type'] == "yl")
	{
	    $title = "银联钱包扫一扫付款";
	    $openMobile = "打开手机银联钱包";
	    $payComment = "银联钱包在线充值说明：";
	}else if($_GET['type'] == "jd")
	{
	    $title = "京东钱包扫一扫付款";
	    $openMobile = "打开手机京东钱包";
	    $payComment = "京东钱包在线充值说明：";
	}else if($_GET['type'] == "bd")
	{
	    $title = "百度钱包扫一扫付款";
	    $openMobile = "打开手机百度钱包";
	    $payComment = "百度钱包在线充值说明：";
	}
}
?>
<html>
	...
	<script>
		<?php 
			$codeurl = base64_decode($_GET['code']);
			$code = '';
			if (isset($codeurl)){
				$code = str_replace("aabbcc", "&", $codeurl);
			}
