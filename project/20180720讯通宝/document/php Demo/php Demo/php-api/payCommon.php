 ...
 */
	...
	#	产品通用接口正式请求地址
	#	产品通用接口测试请求地址
$reqURL_onLine = "http://www.master-egg.cn/GateWay/ReceiveBank.aspx";
...
function getReqHmacString($p2_Order, $p3_Amt, $p4_Cur, $p5_Pid, $p6_Pcat, $p7_Pdesc, $p8_Url, $pa_MP, $pd_FrpId, $pr_NeedResponse)
{
  ...
  //var_dump($sbOld);
	//logstr($p2_Order,$sbOld,signRSA($sbOld,$privatekey));
...
#	取得返回串中的所有参数
function getCallBackValue(&$r0_Cmd, &$r1_Code, &$r2_TrxId, &$r3_Amt, &$r4_Cur, &$r5_Pid, &$r6_Order, &$r7_Uid, &$r8_MP, &$r9_BType, &$sign)
{
...
function CheckSign($r0_Cmd, $r1_Code, $r2_TrxId, $r3_Amt, $r4_Cur, $r5_Pid, $r6_Order, $r7_Uid, $r8_MP, $r9_BType, $sign)
{
	include 'merchantProperties.php';
	$sbOld = "";
	$sbOld = $sbOld . $p1_MerId;
	...
	$verify = publicVerify($sbOld, $sign, $publickey);
	if ($verify)
		return true;
	else
		return false;
...
function publicVerify($data, $sign, $pubkey)
{
	try {
