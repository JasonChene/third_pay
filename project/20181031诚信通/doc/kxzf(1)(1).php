...
$shop_id = "b171ae1dedd945fac11b25874e36af46";
$shop_key = "WJSQpmkYdB4XiyZX";
...
if ($type != 'alipay' && $type != 'wechat') $type = 'alipay'; //默认支付方式 支付宝
$sign = md5($shop_id . $pay_id . $price . $type . $shop_key);
$param = array(
    'shop_id' => $shop_id,
    'user_id' => $pay_id,
    'money' => $price,
...
$resjson = httpspost("https://www.kxzf8.com/api/shopApi/order/createorder", $param);
$res = json_decode($resjson);
var_dump($res);
if(isMB()){
	if($res->type == 'alipay'){
		header("Location:".$res->pay_url); //跳转到支付页面
	}else{
		header("Location:".$res->qrcode_url); //跳转到支付页面
	}
}else{
	header("Location:".$res->qrcode_url); //跳转到支付页面
}
