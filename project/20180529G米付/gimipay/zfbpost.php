...
if (strstr($_REQUEST['pay_type'], "支付宝反扫")) {
  ...
  $data['productType'] = '20000503';//支付宝条形码
}else {
  $scan = 'zfb';
  $bankname = $pay_type."->支付宝在线充值";
  $payType = $pay_type."_zfb";
  $data['productType'] = '20000303';//支付宝扫码
  if (_is_mobile()) {
    $data['productType'] = '20000203';//支付宝wap
  }
}
...
#跳轉方法
$paymessagearr = json_decode($row['payMessage'],1);
if (is_array($paymessagearr)){
  echo '错误代码:' . $paymessagearr['resp_code']."<br>";
  echo '错误讯息:' . $paymessagearr['resp_msg']."<br>";
  exit();
}
if ($row['resultCode'] != '0000') {
  echo  '错误代码:' . $row['resultCode']."<br>";
  echo  '错误讯息:' . $row['payMessage']."<br>";
	exit;
}else {
  if (_is_mobile() && $scan !='zfbf') {
    $jumpurl = $row['payMessage'];
  }elseif ($scan == 'zfbf') {
    echo $row['payMessage'];//返回html格式，直接跳转
    exit();
  }else{
