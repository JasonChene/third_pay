...
$form_url ='https://gateway.gimi321.com/scanPay/initPay';
#第三方参数设置
$data =array(
  'payKey' => $pay_account,//商户支付Key
  'orderPrice' => $mymoney,//	订单金额，单位：元,保留小数点后两位
  'outTradeNo' => $order_no,//商户支付订单号（长度30以内）
  'productType' => '',//支付方式
  'orderTime' => date("YmdHis"),//	下单时间，格式(yyyyMMddHHmmss)
...
if (strstr($_REQUEST['pay_type'], "京东钱包")) {
  ...
  $data['productType'] = '80000103';//京东扫码
  if (_is_mobile()) {
    $data['productType'] = '80000203';//京东wap
  }
}elseif (strstr($_REQUEST['pay_type'], "QQ钱包") || strstr($_REQUEST['pay_type'], "qq钱包")) {
  $scan = 'qq';
  $bankname = $pay_type."->QQ钱包在线充值";
  $payType = $pay_type."_qq";
  $data['productType'] = '70000103';//qq扫码
  if (_is_mobile()) {
    $data['productType'] = '70000203';//qqwap
  }
}elseif (strstr($_REQUEST['pay_type'], "微信反扫")) {
  $scan = 'wxf';
  $payType = $pay_type."_wx";
  $bankname = $pay_type . "->微信在线充值";
  $data['productType'] = '10000503';//微信条形码
}else {
  $scan = 'wx';
  $payType = $pay_type."_wx";
  $bankname = $pay_type . "->微信在线充值";
  //$data['productType'] = '10000103';//微信掃碼(目前沒有掃碼)
  //if (_is_mobile()) {
    $data['productType'] = '10000203';//手机微信
  //}
}
...
if ($row['resultCode'] != '0000') {
  ...
  if ((_is_mobile() &&  $scan=='qq') || $scan=='wx' || $scan =='wxf') {
    $jumpurl = $row['payMessage'];
  }
  elseif (_is_mobile() && $scan=='jd') {
    echo $row['payMessage'];exit();//返回html格式，直接跳转
  }else {
