...
$data = array(
  ...
  "return_url" => $return_url,//通知地址
  "money" => number_format($_REQUEST['MOAmount'], 0, '.', ''),//订单金额：单位/元
  "from_order_id" => $order_no,//商户流水号
  "pay_type" => '4',//支付方式
  "beizhu" => 'yesOhyes'//备注
);
#变更参数设置
$form_url = 'http://www.3vus.com/orderCreate';//提交地址
$scan = 'qq';
if(_is_mobile()){
  $data['pay_type'] = '40';
}
...
if ($row['status'] != '0') {
  ...
  if(_is_mobile()){
    $jumpurl = $row['data']['pay_url'];
  }else{
    $jumpurl = '../qrcode/qrcode.php?type='.$scan.'&code=' .QRcodeUrl($row['data']['qrcode']);
  }
}
#跳轉方法
