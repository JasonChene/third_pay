...
if (strstr($_REQUEST['pay_type'], "微信反扫") || strstr($_REQUEST['pay_type'], "QQ反扫") || strstr($_REQUEST['pay_type'], "qq反扫")) {
  $sscan = (strstr($_REQUEST['pay_type'], "微信反扫")) ? 'wx' : 'qq' ;
  if (!$_POST['authCode']) {
    $data = array();
    ...
    <html>
      ...
      <body>
          <form name="dinpayForm" method="post" id="frm2" action="./fscard.php" target="_self">
              <p>正在为您跳转中，请稍候......</p>
              <input type="hidden" name="file" value="<?php echo $sscan; ?>" />
...
}elseif (strstr($_REQUEST['pay_type'], "QQ反扫")||strstr($_REQUEST['pay_type'], "qq反扫")) {
  $form_url = 'http://103.21.141.192:8080/pay/barcode.do';
  $scan = 'qqf';
...
elseif (strstr($_REQUEST['pay_type'], "京东钱包")) {
  ...
  $scan = 'qq';
  $data['sign']['str_arr']['payType'] = (int)40;
  $data['payType'] = (int)40;
  if(_is_mobile()){
    $form_url = 'http://103.21.141.192:8080/pay/h5.do';
...
#跳转qrcode
if ($row['retCode'] == '00') {
  if (_is_mobile() && $scan != 'bd' && $scan != 'wxf' && $scan != 'qqf'){
    $url = $row['prepayUrl'];
    $jumpurl = $url;
  }elseif ($scan == 'wxf' || $scan == 'qqf') {
