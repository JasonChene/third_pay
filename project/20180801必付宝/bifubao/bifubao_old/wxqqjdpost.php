...
#变更参数设置
$form_url_qr = 'http://s3.av8dpay.com/bifubao-gateway/back-pay/qr-pay';//扫码支付地址
$form_url_h5 = 'http://s3.av8dpay.com/bifubao-gateway/front-pay/h5-pay.htm';//H5支付地址
if (strstr($pay_type, "京东钱包")) {
  $scan = 'jd';
  $data['TYPE'] = '5';//支付方式
  $form_url = $form_url_qr;//扫码支付地址
  if (_is_mobile()) {
    $data['TYPE'] = '10';//支付方式
    $data['RET_URL'] = $return_url;//返回地址
    $form_url = $form_url_h5;//H5支付地址
  }
  ...
  $scan = 'wxfs';
  $data['TYPE'] = '7';//支付方式
  $data['AUTHCODE'] = '';//微信收款码数字
  $form_url = $form_url_qr;//扫码支付地址
  if (isset($_REQUEST['AUTHCODE'])) {
    $data['AUTHCODE'] = $_REQUEST['AUTHCODE'];
  } else {
    ?>
      <html>
        <head>
        ...
        <body>
          <form name="dinpayForm" method="get" id="frm1" action="./card_wxfs.php" target="_self">
            <p>正在为您跳转中，请稍候......</p>
