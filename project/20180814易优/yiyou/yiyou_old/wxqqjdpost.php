...
#固定参数设置
$form_url = 'http://open.eyouc.net/gateway/soa';
...
if (strstr($_REQUEST['pay_type'], "京东钱包")) {
  $scan = 'jd';
  $data['trade_type'] = '010014'; 
} elseif (strstr($_REQUEST['pay_type'], "QQ钱包") || strstr($_REQUEST['pay_type'], "qq钱包")) {
  $scan = 'qq';
  $data['trade_type'] = "010000";
  if (_is_mobile()) {
    $data['trade_type'] = '010006'; 
  }
} else {
  $scan = 'wx';
  $data['trade_type'] = "010001";
  if (_is_mobile()) {
    $data['trade_type'] = '010007'; 
  }
}
...
$jsondata = json_encode($data2,320);
$urljson = urlencode($jsondata);
$data['biz_content'] = base64_encode($urljson);
...
$message = $res->message;
$pay_info = urldecode($res->pay_info);
#跳转
if ($status == '0') {
  if ($result_code == '1' || $result_code == '4') {
    if (_is_mobile()) {
      $jumpurl = $pay_info;
    } else {
      $jumpurl = '../qrcode/qrcode.php?type=' . $scan . '&code=' . QRcodeUrl($pay_info);
    }
  }else {
    echo '错误代码:' . $result_code . "<br>";
    exit;
  }
} else {
  echo '错误代码:' . $status . "<br>";
  echo '错误讯息:' . $message . "<br>";
  exit;
}
#跳轉方法
