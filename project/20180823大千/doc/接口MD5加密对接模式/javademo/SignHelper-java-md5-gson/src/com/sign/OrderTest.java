...
public class OrderTest {
	...
	private static String testOrder(String order_trano_in) {
		...
		// 要发送的测试数据转Json格式
		String jsonString = new GsonBuilder().enableComplexMapKeySerialization().create().toJson(map).toString();
		// 将需要发送的Json数据进行DES加密
	...
	private static TreeMap<String, String> getOrderContentMap(String order_trano_in) {
		...
		map.put("order_trano_in", "441005308f254ef4971bf0c978b25bdf");// 商户单号
		map.put("order_goods", "111");// 商品名称
		map.put("order_ip", "127.0.0.1");// 设备系统版本
		map.put("order_notify_url", "http://localhost:8080/cash/order/daqian/notify/");// 异步通知地址
		map.put("order_return_url", "http://localhost:8080/");// 成功后同步地址
		map.put("order_amount", "1000");// 订单金额，单位分 (不能小于100)
