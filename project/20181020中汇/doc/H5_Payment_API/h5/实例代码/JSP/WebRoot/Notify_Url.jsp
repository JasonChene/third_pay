...
//////////////////////////////////// 异步通知验签  ////////////////////////////////////////////////
		...
		System.out.println("康付通返回的签名参数排序：" + signInfo.length() + " -->" + signInfo);		
		...
		if("RSA-S".equals(sign_type)){ // sign_type = "RSA-S"			
			...
			/**
			1)dinpay_public_key，康付通公钥，每个商家对应一个固定的康付通公钥（不是使用工具生成的商户公钥merchant_public_key，不要混淆），
			     即为康付通商家后台"支付管理"->"公钥管理"->"康付通公钥"里的绿色字符串内容
			2)demo提供的dinpay_public_key是测试商户号123001002003的康付通公钥，请自行复制对应商户号的康付通公钥进行调整和替换	*/		
						
			String dinpay_public_key = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCJQIEXUkjG2RoyCnfucMX1at7OPtOCDSiKZhtzHw5HOjXKteBpYBqEBOZc9pNjP/fKbvBNZ3Z7XxUn5ECfQbPCtH9y++c0WxAYPoZiPDEYeQmRJfqPR68c0aAtZN5Kh7H1SI2ZRvoMUdZGvvFy3vuPnTwm3R+aHq17bch/0ZAudwIDAQAB";
			result = RSAWithSoftware.validateSignByPublicKey(signInfo, dinpay_public_key, dinpaySign);	// 验签   signInfo康付通返回的签名参数排序， dinpay_public_key康付通公钥， dinpaySign康付通返回的签名
		}
		...
		if("RSA".equals(sign_type)){ // 数字证书加密方式 sign_type = "RSA"
			...
			result = mh.validateSignByPubKey(merchant_code, signInfo, dinpaySign);						// 验签    merchant_code为商户号， signInfo康付通返回的签名参数排序， dinpaySign康付通返回的签名
		}
