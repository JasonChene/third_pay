...
// H5選擇type
if($jdscan){
  $trade_type = '92104';//京东扫码
}else{
  if (_is_mobile()) {
   $trade_type = '50107';//微信 WAP
  } else {
    $trade_type = '50104';//微信扫码
  }
...
if($res['http_code'] == 200){
  ...
  if($return_info->respcd == "0000"){
    ...
    if(_is_mobile() && $scan != "jd"){
      ...
      header("location:".'../qrcode/qrcode.php?type='.$scan.'&code='.$purl );
    } 
  }else{
    echo ("第三方错误代码：".$return_info->respcd.'<br>');
    echo ("第三方错误讯息：".$return_info->respmsg.'<br>');
  }
}else{
  echo '发生错误';
}
