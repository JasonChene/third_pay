...
namespace Com.demo
{
    ...
    public class RSACryptoService
    {
        ...
        {
            //string filepath = HttpContext.Current.Server.MapPath("~/key") + "\\rsa_private_key.pem";
            FileStream fs = new FileStream(filepath, FileMode.Open);
            System.Text.UTF8Encoding enc = new System.Text.UTF8Encoding();
            byte[] bytes = new byte[fs.Length];
            fs.Read(bytes, 0, (int)fs.Length);
            string op = enc.GetString(bytes);
            fs.Close();
            return op;
        }
        public static string GetPublicKey(string filepath)
        {
            //string filepath = HttpContext.Current.Server.MapPath("~/key") + "\\rsa_public_key.pem";
            FileStream fs = new FileStream(filepath, FileMode.Open);
            System.Text.UTF8Encoding enc = new System.Text.UTF8Encoding();
            byte[] bytes = new byte[fs.Length];
            fs.Read(bytes, 0, (int)fs.Length);
        ...
        //private string Decrypt3(string ciphertext)
        //{


        //    Byte[] CiphertextData = Convert.FromBase64String(ciphertext);
        //    int MaxBlockSize = _publicKeyRsaProvider.KeySize / 8;    //解密块最大长度限制

        //    if (CiphertextData.Length <= MaxBlockSize)
        //        return System.Text.Encoding.UTF8.GetString(_publicKeyRsaProvider.Decrypt(CiphertextData, false));
        ...
        //            Byte[] Plaintext = _publicKeyRsaProvider.Decrypt(ToDecrypt, false);
        //            PlaiStream.Write(Plaintext, 0, Plaintext.Length);
