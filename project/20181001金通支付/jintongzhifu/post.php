...
$form_url = "http://www.55666g.com/orderpay.do";
#第三方参数设置
$data = array(
  "userId" => $pay_mid, 
  "orderNo" => $order_no,
...
ksort($data);
$noarr = array('sign','bankId','goodsName');
$signtext = '';
...
if ($row['retCode'] != '0') {
    ...
    $signtext2 = 'orderNo='.$row['orderNo'].'&payUrl='.$row['payUrl'].'&retCode='.$row['retCode'].
    '&tradeType='.$row['tradeType'].'&transNo='.$row['transNo'].'&userId='.$row['userId'].'&key='.$pay_mkey;
    $sign2 = md5($signtext2);
    if ($row['sign'] == $sign2) {
        if (($scan == 'yl' && _is_mobile()) || $scan == 'wy') {
            echo $row['payUrl'];
            exit;
        }else {
            $jumpurl = $jumpurl = '../qrcode/qrcode.php?type=' . $scan . '&code=' . QRcodeUrl($row['payUrl']);
        }
    }else{
        echo ('签名不正确！');
        exit;
