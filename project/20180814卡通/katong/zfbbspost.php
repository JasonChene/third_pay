...
$nonce_str = substr(rand(0, 9999) + 10000, 1);
...
#跳转qrcode
$url = json_decode($res['pay_info'], 1);
if ($res['status'] == '0') {
  if ($res['result_code'] == '0' || $res['result_code'] == '00') {
    $qrurl = QRcodeUrl($url['mweb_url']);
    $jumpurl = '../qrcode/qrcode.php?type=zfb&code=' . $qrurl;
  } else {
    echo "错误码：" . $res['err_code'] . "错误讯息：" . $res['err_msg'];
    exit();
  }
} else {
