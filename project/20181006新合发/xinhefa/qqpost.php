...
#变更参数设置
$form_url = 'http://www1.keqiuw.com/pay/createorder';//请求地址
$scan = 'qq';
$data['channel'] = 'qq';
if (_is_mobile()) {
  $data['terminal'] = 'h5';
...
$signtext = $data['pid'].$data['money'].$data['out_order_id'].$data['extend'].$pay_mkey;
$sign = md5($signtext);
$data['sign'] = $sign;
...
if($row['error'] == '0') {
  ...
  sleep(5);
  $query_url = "http://www1.keqiuw.com/pay/Queryorder";
  $data2 = array(
    "pid" => $pay_mid,
    "out_order_id" => $order_no, 
    "sign" => "",
  );
  $data2['sign'] = md5($data2['pid'].$data2['out_order_id'].$pay_mkey);
  $res2 = curl_post($query_url, http_build_query($data2));
  ...
  if($row2['error'] != '0') {
    ...
    if(_is_mobile()) {
      $jumpurl = $row2['data']['payurl'];
      header("location:".$jumpurl);
      exit;
    }else{
      $jumpurl = '../qrcode/qrcode.php?type=' . $scan . '&code=' . QRcodeUrl($row2['data']['payurl']);
    }
  }
}else{
  echo '错误代码:' . $row['error'] . "<br>";
