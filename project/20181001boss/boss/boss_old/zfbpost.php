...
$data = array(
  ...
  "payType" => '', //支付方式
  "amount" => number_format($_REQUEST['MOAmount'] * 100, 0, '.', ''), //金额
  "productName" => 'pay', //商品名称
  "asynNotifyUrl" => $merchant_url, //商户异步通知地址
  "sign" => '', //签名信息
);

#变更参数设置
$form_url = 'http://34.248.37.237:8888/ts/scanpay/pay';//请求地址
$scan = 'zfb';
...
$data_row = json_decode($row['data'], 1);
...
if ($row['code'] != 'success') {
  ...
  exit;
} elseif (!isset($data_row['payUrl'])) {
  echo '错误代码:' . $data_row['code'] . "\n";
  echo '错误讯息:' . $data_row['message'] . "\n";
  exit;
} else {
  $qrcodeUrl = $data_row['payUrl'];
  if (_is_mobile()) {
    $jumpurl = $qrcodeUrl;
  } else {
