...
$content = array(
  ...
  "spbill_create_ip" => getClientIp(), //用户端ip
  "notify_url" => $merchant_url, //异步回调地址
  "return_url" => $return_url, //同步回调地址
);

#变更参数设置
$form_url = 'http://www.miyipay.com/gateway';//请求地址
if (strstr($pay_type, "银联钱包")) {
  ...
  $data['method'] = 'gateway';
  if (_is_mobile()) {
    $content['channel_type'] = '08';//渠道类型
  } else {
    $content['channel_type'] = '07';//渠道类型
  }
  $content['subject'] = 'subject';//订单标题
  $content['bank_code'] = 'ICBC';//银行编码 写死 跳收银台选
}
...
if (strlen($res) < 1000) {
  ...
  ?>
<p hidden="hidden"><?php echo $res; ?></p>
<?php 
}
?>