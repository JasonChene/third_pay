...
-----END PRIVATE KEY-----';
	...
	//merchant_public_key,商户公钥，按照说明文档上传此密钥到智宝付商家后台，位置为"支付设置"->"公钥管理"->"设置商户公钥"，代码中不使用到此变量
...
/**
1)dinpay_public_key，速龙公钥，每个商家对应一个固定的速龙公钥（不是使用工具生成的密钥merchant_public_key，不要混淆），
即为速龙商家后台"公钥管理"->"速龙公钥"里的绿色字符串内容,复制出来之后调成4行（换行位置任意，前面三行对齐），
并加上注释"-----BEGIN PUBLIC KEY-----"和"-----END PUBLIC KEY-----"
2)demo提供的dinpay_public_key是测试商户号800003004321的速龙公钥，请自行复制对应商户号的速龙公钥进行调整和替换。
3）使用速龙公钥验证时需要调用openssl_verify函数进行验证,需要在php_ini文件里打开php_openssl插件
