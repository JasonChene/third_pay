...
////////////////////////////////////  异步通知验签  ////////////////////////////////////
		...
		if("RSA-S".equals(sign_type)){ // sign_type = "RSA-S"			
			...
			/**
			1)dinpay_public_key， 公钥，每个商家对应一个固定的 公钥（不是使用工具生成的商户公钥merchant_public_key，不要混淆），
			     即为 商家后台"支付管理"->"公钥管理"->" 公钥"里的绿色字符串内容
			2)demo提供的dinpay_public_key是测试商户号199001002003的 公钥，请自行复制对应商户号的 公钥进行调整和替换	*/		
			...
			result = RSAWithSoftware.validateSignByPublicKey(signInfo, dinpay_public_key, dinpaySign);	// 验签   signInfo 返回的签名参数排序， dinpay_public_key 公钥， dinpaySign 返回的签名
		}
		...
		if("RSA".equals(sign_type)){ // 数字证书加密方式  sign_type = "RSA"
			...
			result = mh.validateSignByPubKey(merchant_code, signInfo, dinpaySign);						// 验签    merchant_code为商户号， signInfo 返回的签名参数排序， dinpaySign 返回的签名
		}
