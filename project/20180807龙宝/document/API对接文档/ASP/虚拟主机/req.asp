...
'**************************************************
	...
	If HasChinese(p2_Order) Then
		Response.Write "<script>alert('参数中不能包含中文！');history.go(-1);</script>"
		Response.End
	End If
	...
	If Not IsNumeric(p3_Amt) Then
		Response.Write "<script>alert('提交金额非法！');history.go(-1);</script>"
		Response.End	
	End If
...
<body onLoad="document.API.submit()">
	<form name="API" action="<%=reqURL_onLine%>" method="post" target="_self">
