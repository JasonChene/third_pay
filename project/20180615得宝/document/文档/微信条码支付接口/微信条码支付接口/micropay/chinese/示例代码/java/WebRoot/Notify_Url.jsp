...
//////////////////////////////////// 接收参数 request parameters //////////////////////////////////////
		...
		System.out.println("得宝返回的签名字符串：" + signInfo.length() + " -->" + signInfo);		
		System.out.println("得宝返回的签名：" + dinpaySign.length() + " -->" + dinpaySign);								
		...
		if("RSA-S".equals(sign_type)){ //sign_type = "RSA-S"			
			...
			1)dinpay_public_key，得宝公钥，每个商家对应一个固定的得宝公钥（不是使用工具生成的密钥merchant_public_key，不要混淆），
			     即为得宝商家后台"公钥管理"->"得宝公钥"里的绿色字符串内容
			2)demo提供的dinpay_public_key是测试商户号1111110166的得宝公钥，请自行复制对应商户号的得宝公钥进行调整和替换	*/		
			...
			result=RSAWithSoftware.validateSignByPublicKey(signInfo, dinpay_public_key, dinpaySign);	// 验签  signInfo签名字符串，dinpay_public_key得宝公钥，dinpaySign得宝返回的签名数据
		}
		...
		if("RSA".equals(sign_type)){ //数字证书加密方式 sign_type = "RSA"
			...
			// 验签   merchant_code为商户号，signInfo签名字符串，dinpaySign得宝返回的签名数据
