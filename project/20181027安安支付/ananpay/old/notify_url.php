...
#设定固定参数
$order_no = $data['mer_orderid']; //订单号
$mymoney = number_format($data['money'], 2, '.', ''); //订单金额
$status_success_msg = $data['pay_status'];//成功讯息
$status_success_code = "1";//文档上的成功讯息
$pay_success_msg = $data['pay_code'];//成功讯息
$pay_success_code = "SUCCESS";//文档上的成功讯息
$sign = $data['sign'];//签名
...
$iparr = explode('###', $pay_account);
$ip = $_SERVER['REMOTE_ADDR'];
//write_log("ip=" . $ip);
$ipcheck = 0;
foreach ($iparr as $arr_key => $arr_val) {
	if ($arr_val == $ip) {
		$ipcheck = 1;
	}
}
