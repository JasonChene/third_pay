...
if ($scan2 == "h5" && $scan == "wx") {
  $form_url = 'https://api.xhbill.com/gateway/api/h5apipay';  //提交地址
  $interface_version = "V3.1";//接口版本
  $service_type = 'weixin_h5api';
} elseif ($scan == "wxfs") {
  $form_url = 'https://api.xhbill.com/gateway/api/micropay';  //提交地址
  $service_type = 'weixin_micropay';
  $interface_version = "V3.0";//接口版本
  if (isset($_REQUEST['auth_code'])) {
    $auth_code = $_REQUEST['auth_code'];//条码内容
  } else {
    ?>
    <html>
      <head>
  ...
  $form_url = 'https://api.xhbill.com/gateway/api/scanpay';  //提交地址
  $interface_version = "V3.1";//接口版本
  if ($scan == "wx") {
    $service_type = 'weixin_scan';
  } elseif ($scan == "jd") {
    $service_type = 'jd_scan';
  }
}
if ($scan == 'qq') {
  if ($scan2 == "h5") {
    $form_url = 'https://api.xhbill.com/gateway/api/h5apipay';  //提交地址
    $interface_version = "V3.1";//接口版本
    $service_type = 'qq_h5api';
  } else {
    $form_url = 'https://api.xhbill.com/gateway/api/scanpay';  //提交地址
    $service_type = 'qq_scan';
    $interface_version = "V3.1";//接口版本
  }
}
