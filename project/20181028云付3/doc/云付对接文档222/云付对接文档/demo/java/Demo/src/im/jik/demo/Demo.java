...
public class Demo {
	...
	public static void main(String[] args) {
		...
		String sign = null;
		String p0 = price.substring(0, price.indexOf("."));
		String p1 = price.substring(price.indexOf("."), price.length());
		int p0i = Integer.parseInt(p0);
		int s0 = p0i / 1000;
		String str = null;
		if(p0.length()>=5) {
			String s1 = price.substring(2, price.indexOf("."));
			str = String.valueOf(s0)+","+s1+p1;
		} else if(p0.length()>=4) {
			String s1 = price.substring(1, price.indexOf("."));
			str = String.valueOf(s0)+","+s1+p1;
		} else {
			str = price;
		}
		//System.out.println(str);
		sign = Demo.getSign(str, "201806261212440");
		...
		*/
		String order = Demo.post("http://47.105.34.228/gateway/index/checkpoint.do", params);
