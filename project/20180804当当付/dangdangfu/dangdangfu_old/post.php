...
$data = array(
  "merchno" => $pay_mid, //商户号
  "amount" => number_format($_REQUEST['MOAmount'], 0, '.', ''), //交易金额
  "traceno" => $order_no, //商户流水号
  "notifyUrl" => $merchant_url, //商户通知地址
  "settleType" => '', //结算方式
  "signature" => '', //数据签名
);
...
if (strstr($pay_type, "银联钱包")) {
  $scan = 'yl';
  $data['payType'] = '32';//支付方式
  $data['settleType'] = '1';//结算方式
  $form_url = 'http://api.wachou.top/passivePay';//二维码支付提交地址
} elseif (strstr($pay_type, "银联快捷")) {
  $scan = 'ylkj';
  $data['interType'] = '2';//接口类型
  $data['cardType'] = '1';//卡类型
  $data['settleType'] = '2';//结算方式
  $data['returnUrl'] = $return_url;
  $form_url = 'http://api.wachou.top/h5Quick.do';//快捷支付提交地址
} else {
  $scan = 'wy';
  $data['channel'] = '2';
  ...
  $data['returnUrl'] = $return_url;
  $data['settleType'] = '2';//结算方式
  $form_url = 'http://api.wachou.top/gateway.do?m=order';//网银支付提交地址
}
