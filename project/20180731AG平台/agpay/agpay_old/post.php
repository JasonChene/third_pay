...
#第三方参数设置
$data = array(
  "pay_memberid" => $pay_mid, //商户编号
  "pay_orderid" => $order_no, //商户订单号
  "pay_amount" => number_format($_REQUEST['MOAmount'], 0, '.', ''), //订单金额
  "pay_service" => '', //支付类型
  "pay_notifyurl" => $merchant_url, //后台通知
  "pay_applydate" => date("Y-m-d H:i:s"), //订单时间
  "pay_md5sign" => '', //签名数据
);

#变更参数设置
$form_url = 'https://www.agpay88.com/Pay_Index.html';//接口地址
if (strstr($pay_type, "银联快捷")) {
  $scan = 'ylkj';
  $data['pay_service'] = '911';
  $data['pay_callbacku'] = $return_url;//前台通知
} else {
  $scan = 'wy';
  $data['pay_service'] = '907';
  $data['pay_callbacku'] = $return_url;//前台通知
  $data['pay_callbackurl'] = $_REQUEST['bank_code'];//银行代码
}
