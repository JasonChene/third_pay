...
函数:
	...
	recive()
		接收联瑞消息，这里会进行身份的验证，如果您自己要做更多后续的处理，请在您自己的程序中进行
		调用示例：
...
class lianruipay{
	const obao_card_url			  = 'http://zf.lianruipay.com/cardReceive.ashx';
	const obao_card_search_url	= 'http://zf.lianruipay.com/search.ashx';
	...
	*/
	public function send(){	
		//检查是否正确
		$error 	= 0;
		$msg		= '您调用联瑞支付接口的参数有误，错误信息如下：';
		if(empty($this->parter)){
			$error 	= 1;
			$msg 	.= '<li>parter不能为空: 商户id，由联瑞联盟分配</li>';
		}
		if(empty($this->type)){
			$error 	= 1;
		...
		$url	= ekapay::obao_card_url . "?" . $url . "&sign=" .$sign;
	...
	*/
	public function search($order_id){
		...
		//签名
		$sign	= md5($url. $this->key);
		$url	= ekapay::obao_card_search_url . "?" . $url . "&sign=" .$sign;		
		$result=file_get_contents($url);
		parse_str($result, $output);
