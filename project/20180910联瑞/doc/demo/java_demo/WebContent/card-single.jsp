...
String sign = lianruipaypayEncrypt.lianruipaypayCardMd5Sign(type,parter,cardno,cardpwd,value,restrict,    //签名
...
//将联瑞支付上行结果转换为对应中文说明
String strResult = lianruipaypayTypeConvert.opstateValueToChn(result); 
//将卡类型转换为中文说明
String chnType = lianruipaypayTypeConvert.cardTypeToChn(type); 

%>
	<div>联瑞支付卡类接口演示(单卡)</div>
			<div>卡类型:<%=chnType%></div>
			<div>卡号:<%=cardno%></div>
			<div>密码:<%=cardpwd%></div>
			<div>面值:<%=value%></div>
