...
//////////////////////////////////// 异步通知验签  ////////////////////////////////////////////////
		...
		System.out.println("赢宝付返回的签名参数排序：" + signInfo.length() + " -->" + signInfo);		
		System.out.println("赢宝付返回的签名：" + dinpaySign.length() + " -->" + dinpaySign);								
		boolean result = false;
		
		if("RSA-S".equals(sign_type)){ // sign_type = "RSA-S"			
			
			/*
			1)dinpay_public_key，赢宝付公钥，每个商家对应一个固定的赢宝付公钥（不是使用工具生成的商户公钥merchant_public_key，不要混淆），
			     即为赢宝付商家后台"支付管理"->"公钥管理"->"赢宝付公钥"里的绿色字符串内容
			2)demo提供的dinpay_public_key是测试商户号100100200200的赢宝付公钥，请自行复制对应商户号的赢宝付公钥进行调整和替换	*/		
			...
			result = RSAWithSoftware.validateSignByPublicKey(signInfo, dinpay_public_key, dinpaySign);	// 验签   signInfo赢宝付返回的签名参数排序， dinpay_public_key赢宝付公钥， dinpaySign赢宝付返回的签名
		}
				
		if("RSA".equals(sign_type)){ // 数字证书加密方式 sign_type = "RSA"
			
			// 请在商家后台"支付管理"->"证书下载"处申请和下载pfx数字证书，一般要1~3个工作日才能获取到，200001001001.pfx是测试商户号200001001001的数字证书
			String webRootPath = request.getSession().getServletContext().getRealPath("/");
			String merPfxPath = webRootPath + "pfx/200001001001.pfx"; 									// 商家的pfx证书文件路径
			String merPfxPass = "87654321";			  													// 商家的pfx证书密码,初始密码是商户号
			RSAWithHardware mh = new RSAWithHardware();						
			mh.initSigner(merPfxPath, merPfxPass);		  							
			result = mh.validateSignByPubKey(merchant_code, signInfo, dinpaySign);						// 验签    merchant_code为商户号， signInfo赢宝付返回的签名参数排序， dinpaySign赢宝付返回的签名
		}
