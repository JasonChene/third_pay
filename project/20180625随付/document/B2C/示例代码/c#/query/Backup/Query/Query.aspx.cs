            ...
            try
            {
                ...
                if (sign_type == "RSA-S") //RSA-S签名方法
                {
				   ...
				   */
                    ...
                    //用HttpPost方式提交
                    string _xml = HttpHelp.httppost("https://query.fastbank.net/query", para, "UTF-8");
                    //将返回的xml中的参数提取出来
                    ...
                    /**
                     1)zhf_public_key，快银支付公钥，每个商家对应一个固定的快银支付公钥（不是使用工具生成的密钥merchant_public_key，不要混淆），
                     即为快银支付商家后台"公钥管理"->"快银支付公钥"里的绿色字符串内容
                     2)demo提供的zhf_public_key是测试商户号100100101102的快银支付公钥，请自行复制对应商户号的快银支付公钥进行调整和替换。
                     */

                    string zhf_public_key = "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCuuD2ED73Gu3Y3+9KsNJvyCct38GEBCFTL1/lIUhgnWk4VWH6Cyh/fWbSwu0KN/q9cR5Y9dRbpsNnIH9xV2TRR+cUXZNHkqTsr/BADYeMZ9ia2Bt9ry+8kRJzjAggTpsdbazyV1COQwUnsbPF4ezC+8Cku58hjUoGE89mOvNJZuQIDAQAB";
                ...
                else  //RSA签名方法
                {
                    ...
                    //组装字符串
                    string signsrc = "merchant_code=" + merchantcode + "&order_amount=" + orderamount + "&order_no=" + orderno + "&order_time=" + ordertime + "&trade_no=" + trade_no + "&trade_status=" + trade_status + "&trade_time=" + trade_time;
                    bool result = rsaWithH.VerifySign("100100101102", signsrc, zhfsign);
                    if (result == false)
                    {
                        Response.Write("验签失败");
                        Response.End();
