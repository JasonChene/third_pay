...
//////////////////////////////////// 接收参数 request parameters //////////////////////////////////////
		...
		// 接收中返回的参数(To receive the parameter)
		...
		*/
		...
		if("RSA-S".equals(sign_type)) {			
			...
			// 验签  signInfo签名字符串，RSA_S_PublicKey公钥，dinpaySign返回的签名数据
			result=RSAWithSoftware.validateSignByPublicKey(signInfo, dinpay_public_key, dinpaySign);
		}else{
			
			String merPfxPath = "D:/800004007888.pfx";  // 商家的pfx证书文件位置(公私钥合一)
			String pfxPass = "800004007888";			  // 商家的pfx证书密码
			RSAWithHardware mh = new RSAWithHardware();						
			mh.initSigner(merPfxPath, pfxPass);		  
			// 验签   merchantId为商户号，signInfo签名字符串，dinpaySign返回的签名数据
